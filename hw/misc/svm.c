...
#define CHECK_ENTRY_PERM(entry, expected) \
    (((entry)->perm & (expected)) == (expected))
...
static int svm_issue_pri_request(SVMState *svm, hwaddr addr, bool lpig)
{
    ...
    int ret = pci_pri_request_page(&svm->pdev, svm->pasid,
                                   svm->command->priv,  false, addr,
                                   lpig, svm->prgi,
                                svm->command->dir == DMA_DIRECTION_TO_DEVICE,
                                svm->command->dir == DMA_DIRECTION_FROM_DEVICE);
    SVM_LOG("PRI : pasid = %d, addr = 0x%" HWADDR_PRIx ", lpig = %d, "
                                 ...
                                 size_t size, IOMMUAccessFlags expected_perm)
{
    ...
    for (size_t i = 0; i < size; ++i) {
        entry = entries + i;
        if (!CHECK_ENTRY_PERM(entry, expected_perm)) {
            result += 1;
            if ((entry->perm & IOMMU_RW) != IOMMU_NONE) {
                SVM_LOG("Lack of permissions 0x%" HWADDR_PRIx ", expected : "
                        "%u, actual %u\n", entry->iova, expected_perm,
                        entry->perm);
            }
        } else {
            SVM_LOG("Translation for 0x%" HWADDR_PRIx " : 0x%" HWADDR_PRIx "\n",
...
static void svm_prepare_translations(SVMState *svm)
{
    ...
    ret = pci_ats_request_translation(&svm->pdev, svm->pasid,
    ...
    if (ret < 0) { /* Not -ENOMEM as the buffer is large enough*/
        SVM_LOG("Error during ATS request : %" PRIi64 "\n", (int64_t)ret);
        svm->command->failure = true;
        return;
    }
    SVM_LOG("Send ATS request : pasid = %d, addr = 0x%" HWADDR_PRIx ", "
            "length = %" PRIu64 ", no_write = %d\n", svm->pasid, addr,
            (uint64_t)length, no_write);
    SVM_LOG("Translation errors : %d / %" PRIi64 "\n", err_count, (int64_t)ret);
    pri_gp_len = svm_try_update_atc(svm, buffer, ret, expected_perm);
    assert(pri_gp_len == err_count);

    for (ssize_t i = 0; i < ret; ++i) {
        if (!CHECK_ENTRY_PERM(buffer + i, expected_perm)) {
            pri_req_count += 1;
...
static void svm_iommu_notification_handler(IOMMUNotifier *notifier,
                                           IOMMUTLBEntry *data)
{
    SVMState *svm = container_of(notifier, SVMState, iommu_notifier);
    SVM_LOG("Invalidation : addr = 0x%" HWADDR_PRIx " - pasid = %x - mask = 0x%"
...
static void svm_disable_pasid(SVMState *svm)
{
    ...
    pci_iommu_unregister_iotlb_notifier(&svm->pdev, svm->pasid,
                                            &svm->iommu_notifier);
...
static void svm_enable_pasid(SVMState *svm, uint32_t pasid)
{
    ...
    if (ret != 0) {
        SVM_LOG("Failed to register PRI notifier\n");
        return;
    }
    ...
    svm->pasid = pasid;
    ret = pci_iommu_init_iotlb_notifier(&svm->pdev, &svm->iommu_notifier,
                                        svm_iommu_notification_handler, NULL);
    if (ret != 0) {
        SVM_LOG("Failed to init IOTLB notifier");
        return;
    }

    ret = pci_iommu_register_iotlb_notifier(&svm->pdev, svm->pasid,
                                            &svm->iommu_notifier);
    if (ret != 0) {
        SVM_LOG("Failed to register IOTLB notifier");
        return;
    }
...
static void svm_init_atc(SVMState *svm)
{
    ...
    rc = pci_iommu_get_iotlb_info(&svm->pdev, &address_width,
                                  &svm->min_page_size);
    assert(rc == 0);
...
static void svm_do_process_command(SVMState *svm)
{
    ...
    while (command->cur_addr < virt_end) {
        ...
        if (iotlb_entry->perm & IOMMU_UNTRANSLATED_ONLY) {
            SVM_LOG("SVM untranslated only : 0x%" HWADDR_PRIx "\n",
        ...
        for (i = 0; i < chunk_size; i += sizeof(buf)) {
            ...
            if (svm->command->dir == DMA_DIRECTION_FROM_DEVICE) {
                command->first_value = svm_prepare_sub_buffer(buf,
                                                        command->first_value,
                                                        sub_chunk_size);
                pci_dma_rw(&svm->pdev, phys_addr, buf, sub_chunk_size,
                           DMA_DIRECTION_FROM_DEVICE,
                           (MemTxAttrs) { .address_type = PCI_AT_TRANSLATED });
            } else {
                pci_dma_rw(&svm->pdev, phys_addr, buf, sub_chunk_size,
...
static void svm_handle_done(SVMState *svm)
{
    ...
    if (svm->command->dir == DMA_DIRECTION_FROM_DEVICE) {
        ...
        SVM_LOG("CRC : Expected = %" PRIx8 " - Actual = %" PRIx8 "\n",
...
static void svm_class_init(ObjectClass *class, const void *data)
{
