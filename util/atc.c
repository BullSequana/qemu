...
{
    hwaddr addr = (hwaddr)(uintptr_t)v;
...
{
    return (hwaddr)(uintptr_t)v1 == (hwaddr)(uintptr_t)v2;
}
...
int atc_update(ATC *atc, IOMMUTLBEntry *entry)
{
    ...
    g_hash_table_replace(as_cache, (gpointer)(uintptr_t)(entry->iova), value);
    return 0;
}
...
IOMMUTLBEntry *atc_lookup(ATC *atc, uint32_t pasid, hwaddr addr)
{
    ...
    for (uint8_t level = 0; level < atc->levels; ++level) {
        entry = g_hash_table_lookup(as_cache, (gconstpointer)(uintptr_t)key);
